function VACC_TheVision(dataDir, fiftyNineBad, varargin)
...
%
% Example (with bad channel 59 dropped and custom width):
%    VACC_TheVision("D:\PTEN\...", true, 'halfWidthMs', 60);

%% -------- Args --------
p = inputParser;
p.addRequired('dataDir', @(s)ischar(s)||isstring(s));
p.addRequired('fiftyNineBad', @islogical);
p.addParameter('halfWidthMs', 50, @(x)isfinite(x)&&x>0);
p.addParameter('minCh', 1, @(x)isfinite(x)&&x>=0);
p.addParameter('maxCh', 64, @(x)isfinite(x)&&x>=0);
p.addParameter('evenOnly', false, @(x)islogical(x));
p.addParameter('invertPolarity', true, @(x)islogical(x));
p.addParameter('loadMaxCh', 64, @(x)isfinite(x)&&x>0); % <-- Caps the loaded files at 64
p.parse(dataDir, fiftyNineBad, varargin{:}); 
...
fiftyNineBad   = p.Results.fiftyNineBad; 
loadMaxCh      = p.Results.loadMaxCh;
...
addpath(codeRoot);
addpath(fullfile(codeRoot, 'reqsPath'));
...
%% -------- Load ets / ech --------
load(fullfile(dataDir, 'ets_hippocampus_59rm_hp.mat'),'ets');
load(fullfile(dataDir, 'ech_hippocampus_59rm_hp.mat'), 'ech');
...
% --- Handle fiftyNineBad parameter ---
if fiftyNineBad
    idx_of_59 = find(nums == 59); 
    if ~isempty(idx_of_59)
        fprintf('fiftyNineBad=true. Dropping CSC59 from analysis entirely.\n');
        nums(idx_of_59) = NaN; % Mark for removal
    else
        fprintf('fiftyNineBad=true, but CSC59.ncs not found in file list. No action needed.\n');
    end
end
% --- END ---

keep = ~isnan(nums);
keep = keep & (nums <= loadMaxCh); % <-- Drop any channels > loadMaxCh
...
%% -------- Select events --------
% Note: ech(:,1:nCh) naturally restricts checking to only the loaded channels
...
outDir = fullfile(codeRoot, 'Output', animalName);
