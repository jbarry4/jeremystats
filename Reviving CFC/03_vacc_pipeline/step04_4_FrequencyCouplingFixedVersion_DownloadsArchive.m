function []=step04_4_FrequencyCouplingFixedVersion_DownloadsArchive ()
...
cd (path_xls);
[data,txt] =  xlsread('ELS4S2Feeder.xlsx');
qqq=txt(1:2,:);
txt(1:2,:)=[];
...
%Set new WD:
%You must make Comodulogram a cell before you bind!
Comodulogram = Comodulogram';
Comodulogram = num2cell(Comodulogram);
Comodulogram = [Comodulogram,IdClear,GroupClear,LayerClear];
newcd = 'D:\PupProbePilot\Matlab Code for amplitude and frequency and feeder sheet\CFC';
cd (newcd);
    new_name = [ratId, '_Comodulogram_',num2str(eegnum)];
    new_name2 = [ratId,'_ComodFig_',num2str(eegnum)]; 
