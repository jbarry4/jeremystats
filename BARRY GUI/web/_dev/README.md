...
| `gain.html` | that the gain applies to every pane of a recording and not only the one whose control was touched — read off the canvas pixels of each pane, because `sess.gain` was always correct and the picture was not |
| `arc.html` | The Arc: the bundle's steps and their construction cards, Spark reading and pairing a real recording, the line view drawing dropped pulses as well as kept ones, and the contract of all three of its Xplorefinder modes: that both bundles render, that the four unbuilt steps are shown, disabled and carry their reason, that none of the five leaked into the flat tool list, that each paints a step header saying which of five it is, and that the Coupling mode exports the whole contract and refuses cleanly |
...
| `vaccopen.html` | opening a recording that is on the cluster and not on this disk: that the two affirmative states offer an Open and the other two do not, that a recording nobody has established an answer for offers NEITHER an Open nor a refusal, that the cost is stated before it is spent, that the palette can find it by either word, and that looking at the link does not open one. The assertion the page exists for is that opening one WRITES NOTHING: `REG.ensure` files every path a recording is opened from, and a `/gpfs2/...` path filed as if it were local reads back as `local-only` -- so the recording would report that the cluster cannot reach it on the strength of having just been read off the cluster, quietly, in a shard that travels to everybody. Runs with the cluster DOWN; the checks that need a live link are skipped in their own colour, so a sweep that proved less than a full one cannot read as a clean one |
| `vaccshot.html` | poses "Everything VACC knows" on its card view for a screenshot: `?tab=catalogue` for the tree, `?tab=browse` for the directory walk. A shot-taker, not a harness — it exists because a shelf of cards is a thing you have to look at, and `--dump-dom` will happily report a layout that has collapsed into one column |
