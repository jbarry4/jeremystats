function CSD_TimeAvgSlices_Waveforms_AvgGroups(inputFolder, dataMatPath, varargin)
% CSD_TimeAvgSlices_Waveforms_AvgGroups
% Like CSD_CenterSlices_Waveforms_AvgGroups, but instead of taking the 0 ms slice,
% it averages the CSD over a time window (default: -5 ms to +15 ms) per channel/event.
%
% LEFT: per-event time-averaged CSD columns (channels x events, column-thickened)
% RIGHT: vertical waveforms (gray = each event’s per-channel average; black = mean)
% Channel 1 is at the TOP. Panels share Y-axis ticks/limits/fonts; shared colorbar.
%
% OUTPUT:
...
% NEW: time-avg window relative to anchor (within the ±winHalfWidthMs capture)
p.addParameter('avgStartMs',  -5e-3, @(x)isfinite(x));             % start of avg window (default -5 ms)
p.addParameter('avgEndMs',    15e-3, @(x)isfinite(x));             % end of avg window   (default +15 ms)
...
% ============================= HELPERS =============================
    ...
    function buildAndRender(evtList, tag, outPath)
        ...
        for ii = 1:numel(evtList)
            ...
            % TIME-AVERAGE over [i0..i1] (indices already bounded)
        ...
        xlim(ax2, [-clim, +clim]);
        xlabel(ax2, sprintf('CSD (avg %+.1f to %+.1f ms)\n(− sink   |   + source)', 1e3*avgStartMs, 1e3*avgEndMs));

        % Enforce identical inner boxes & link Y
        set([ax1 ax2], 'LooseInset', max(get(ax1,'TightInset'), get(ax2,'TightInset')));
        linkaxes([ax1 ax2], 'y');

        % Shared colorbar (attach to ax1, move into layout; fallback if needed)
        axes(ax1);
        cb = colorbar; % attach to ax1
        try
            cb.Layout.Tile = 'east';
        catch
            set(cb, 'Location','eastoutside', 'Box','on');
        ...
        title(ax1, sprintf('%s — TIME-AVG CSD [%+.1f,%+.1f] ms (n=%d)', ...
            tag, 1e3*avgStartMs, 1e3*avgEndMs, nEvt), 'FontSize',10, 'FontWeight','bold');
        title(ax2, sprintf('%s — Vertical waveform (mean in black)', tag), 'FontSize',10, 'FontWeight','bold');
        sg = sprintf('%s  |  align: first-channel max (\\pm%.1f ms)  |  capture: \\pm%.1f ms  |  channels=%d', ...
...
% Interior: standard 3-point stencil; edges replicated from nearest interior.
    ...
    elseif nCh == 2
        C = zeros(size(Ych_t)); return; % degenerate but not blank
    end
