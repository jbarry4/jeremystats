...
% Spectrogram_Waveform_Stacked_FirstEvent
...
%       - Row (2k):   spectrogram (power in dB, 0..fMaxHz), shared CLim across rows
%   • Save one PNG per group in: "<inputFolder>/Spectrogram Waveform Stacked Output/"
%
...
p.addParameter('specWinMs',   20e-3, @(x)isfinite(x)&&x>0);   % <-- default 20 ms
p.addParameter('specOverlap', 0.50,  @(x)isfinite(x)&&x>=0&&x<1);
p.addParameter('nfft',        [],    @(x) isempty(x) || (isscalar(x)&&x>0));
p.addParameter('fMaxHz',      2000,  @(x)isfinite(x)&&x>0);   % <-- y-axis up to 2 kHz
...
% ======================================================================
    function renderOne(e, tag, outDir)
        ...
        tRelMs = (-HW:HW) / sfx * 1e3;   % for waveform x-axis
        centerIdx = HW + 1;
        ...
        % --- Spectrogram params ---
        specWinSamp     = max(8, round(specWinMs * sfx));             % ensure ≥8 samples
        ...
        % --- Figure (2*nCh rows; odd=row waveform, even=row spectrogram) ---
        rows = 2*nCh; cols = 1;
        perRowPx = 90; basePx = 200; maxPx = 5200;
        figH = min(maxPx, basePx + perRowPx * rows);
        f = figure('Color','w','Position',[60 60 980 figH],'Visible','off');
        tl = tiledlayout(f, rows, cols, 'Padding','compact','TileSpacing','compact');
        ...
        for ci = 1:nCh
            ...
            ylim(ax1, yL_global);
            if ci < nCh, ax1.XTickLabel = []; else, xlabel(ax1,'Time (ms)'); end
            ylabel(ax1, '\muV');
            title(ax1, sprintf('%s — waveform', chanLabels{ci}), 'FontSize',9, 'FontWeight','normal');
            ax1.FontSize = 8; ax1.TickDir='out';
            ...
            if ci < nCh, ax2.XTickLabel = []; else, xlabel(ax2,'Time (ms)'); end
            if ci==1
                ylabel(ax2, 'Hz');
            else
                ax2.YTickLabel = [];
            ...
            ax2.FontSize = 8; ax2.TickDir='out';
        end

        % One colorbar for the spectrograms (attach to layout)
        cb = colorbar; set(cb,'Location','eastoutside');
        cb.Label.String = sprintf('Power (dB) | CLim [%.1f, %.1f]', pLo, pHi);

        sg = sprintf('%s  |  Evt %d  |  anchor: first-ch max (\\pm%.1f ms)  |  Window: \\pm100 ms  |  STFT win=%.1f ms ov=%.0f%% nfft=%d  |  chans=%s', ...
                     tag, e, 1e3*HWanchor/sfx, 1e3*specWinSamp/sfx, 100*specOverlapSamp/specWinSamp, nfft, mat2str(chSel));
        sgtitle(tl, sg, 'FontSize',10, 'FontWeight','bold');

        if ~exist(outDir,'dir'), mkdir(outDir); end
        outPng = fullfile(outDir, sprintf('Evt%03d_SpecWave_Stacked.png', e));
