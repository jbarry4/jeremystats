function VoltageRaster_AvgGroups(inputFolder, dataMatPath, varargin)
% VoltageRaster_AvgGroups
% Build ONE voltage raster per group (SOLID, SPUTTER) by averaging aligned
% waveforms across all events in each group.
%
...
% - Averaging: per channel, average across events at each time sample.
% - Output: 2 PNGs with a single GLOBAL color scale (CLim) across both groups.
%
% OUTPUT FILES:
%   <inputFolder>/Voltage Raster Output/Raster_Avg_SOLID.png
%   <inputFolder>/Voltage Raster Output/Raster_Avg_SPUTTER.png
%
...
%   'metricHalfWidthMs' : metrics half-width for peak/HW on mean waveform (default 5e-3)
%   'anchorHalfWidthMs' : anchor search half-width (default 5e-3)
%   'climMicroV'        : fixed symmetric CLim ±µV; if empty, auto global robust (default [])
...
%
% EXAMPLE
%   VoltageRaster_AvgGroups('C:\Exp', 'C:\Exp\data.mat');
...
fprintf('AvgGroups: sfx=%.1f Hz | window ±%.1f ms | anchor: firstCh max (±%.1f ms)\n', ...
...
if isempty(S_mu) && isempty(P_mu)
    warning('No average rasters created (no valid events in either group).');
    return;
end
...
if ~isempty(S_mu)
    renderAvgRaster(S_mu, 'SOLID', outSOLpng, climGlobal, S_stats);
end
if ~isempty(P_mu)
    renderAvgRaster(P_mu, 'SPUTTER', outSPUpng, climGlobal, P_stats);
end
...
function [MU, stats] = buildAvg(evtList, tag)
    ...
    % For per-channel metrics on the MEAN waveform later
    perCh_amp = nan(nCh,1);
    perCh_hw  = nan(nCh,1);
    ...
    MU = sumY / nUsed; % mean waveform per channel
    stats.nEvents = nUsed;

    % ---- Per-channel metrics on the MEAN waveform (±metric window) ----
    for k = 1:nCh
        mu = MU(k,:);
...
function renderAvgRaster(MU, tag, outPath, clim, stats)
