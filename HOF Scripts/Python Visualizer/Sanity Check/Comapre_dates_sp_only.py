...
try:
    ...
    # ENFORCE FILTER: Keep only sessions containing '_SP_'
    global_sessions = global_sessions[global_sessions['Session'].str.contains('_SP_')]
...
# 3. Merge, Filter, Output
merged = pd.merge(med_grouped, global_grouped, on='Date', how='outer').fillna('None')

# Drop dates after the 15th per your directive
merged = merged[merged['Date'] <= '2025-09-15']
