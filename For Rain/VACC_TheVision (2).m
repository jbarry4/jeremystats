function VACC_TheVision(dataDir, eightBad, varargin)
...
%
% Example:
%    VACC_TheVision("D:\PTEN\PTEN\M13_pten\HF4s\IED DATA", false);
%
% Example (with bad channel 8 and custom width):
%    VACC_TheVision("D:\PTEN\...", true, 'halfWidthMs', 60);

%% -------- Args --------
p = inputParser;
p.addRequired('dataDir', @(s)ischar(s)||isstring(s));
p.addRequired('eightBad', @islogical); % <-- NOW A REQUIRED ARG
...
p.parse(dataDir, eightBad, varargin{:}); % <-- UPDATED PARSE CALL
...
eightBad       = p.Results.eightBad; % <-- STILL GETS FROM RESULTS

fprintf('\n=== VACC_TheVision ===\n');

% ---- paths (Linux) ----
codeRoot = fileparts(mfilename('fullpath'));
addpath(codeRoot);                                     % this file's folder
addpath('/gpfs2/scratch/sakhava1/Batch_Process_All/reqsPath');  % << your Linux Nlx2MatCSC

% sanity checks so we fail fast with a clear message
nlxPath = which('Nlx2MatCSC');
if isempty(nlxPath)
    error('Nlx2MatCSC not found. Make sure reqsPath is on the MATLAB path.');
end
fprintf('Nlx2MatCSC resolved to: %s\n', nlxPath);
...
% --- Handle eightBad parameter ---
idx8_replaced_flag = false;
idx_of_eight = find(nums == 8, 1); % Find index of CSC8

if eightBad && ~isempty(idx_of_eight)
    fprintf('eightBad=true. Found CSC8 at index %d.\n', idx_of_eight);
    file9_info = dir(fullfile(dataDir, 'CSC9.ncs'));
    
    if ~isempty(file9_info)
        fprintf('  Found CSC9.ncs. Performing replacement.\n');
        files(idx_of_eight) = file9_info(1); % Replace file info
        nums(idx_of_eight) = 9;            % Replace number
        idx8_replaced_flag = true;
        
        % Now, find if CSC9.ncs existed elsewhere and nullify it
        % We want the one that is NOT idx_of_eight
        idx_of_nine = find(nums == 9);
        original_idx_of_nine = setdiff(idx_of_nine, idx_of_eight);
        
        if ~isempty(original_idx_of_nine)
            nums(original_idx_of_nine) = NaN; % Mark for removal
            fprintf('  Original CSC9.ncs at index %d will be ignored to prevent duplicates.\n', original_idx_of_nine);
        end
    else
        fprintf('  CSC9.ncs not found. CSC8 will be used as-is.\n');
    end
elseif eightBad
    fprintf('eightBad=true, but CSC8.ncs not found in file list. No action taken.\n');
end
% --- END ---

keep = ~isnan(nums);
if evenOnly
    even_mask = (mod(nums,2)==0);
    if idx8_replaced_flag
        even_mask(idx_of_eight) = true; % Force-keep the replaced channel 9
    end
    keep = keep & even_mask;
end
